...
class QuestGameTeamStats extends generated\QuestGameTeamStats
{
    ...
    private static function addIncorrectAnswer(int $tourId, string $userAnswer)
    {
        $stat = new self();
        $stat->tour_id = $tourId;
        $stat->u_id = Session::getUserId();
        $stat->team_id = TeamToUser::getTeamFromUser(Session::getUserId());
    ...
    private static function addCorrectAnswer(int $tourId, string $userAnswer, array $answers)
    {
        ...
        foreach ($questionsId as $questionId) {
            ...
            if (self::getDublicate($tourId, $questionId)) {
                ...
                $stat->is_correct = 1;
                self::updateRemainingTime($gameId, $tourId, $teamId, $questionId);
            }
    ...
    public static function getCorrectAnswers(): array
    {
        ...
        $answerList =  self::find()->select('question_id')->where([
            'quest_game_team_stats.game_id' => $gameId,
            'quest_game_team_stats.team_id' => TeamToUser::getTeamFromUser(Session::getUserId()),
            'quest_game_team_stats.is_correct' => 1
        ])->all();
    ...
    private static function updateRemainingTime(int $gameId, int $tourId, int $teamId, int $questionId): void
    {
        ...
        \Yii::$app->getDb()->createCommand('UPDATE `quest_game_team_tour` SET created_at = DATE_ADD(created_at, INTERVAL '.$bonus.' second) WHERE game_id = '.$gameId.' AND tour_id = '.$tourId.' AND team_id = '.$teamId)->execute();
    }
}